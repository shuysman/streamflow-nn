...
3. area_gages2 matches drainage area used in conversion
...
# Expected drainage areas from streamflow conversion
DRAINAGE_AREAS = {
    'lamar': 1730.1,
    'hoh': 655.3
}
...
def test_attributes(watershed):
    ...
    # Test 8: Check area_gages2 matches drainage area
    area_from_attrs = df['area_gages2'].values[0]
    expected_area = DRAINAGE_AREAS[watershed]
    assert abs(area_from_attrs - expected_area) < 0.01, \
        f"❌ Drainage area mismatch: {area_from_attrs} != {expected_area}"
    print(f"✅ Drainage area matches conversion: {area_from_attrs} km²")
    ...
    assert 0 <= slope < 90, f"❌ Slope out of range: {slope}"
    ...
    if watershed == 'lamar':
        # Lamar is high elevation, moderate slope, moderate forest
        assert elev > 2000, f"❌ Lamar should be high elevation: {elev}"
        assert slope < 15, f"❌ Lamar should have moderate slope: {slope}"
        assert forest < 0.5, f"❌ Lamar has limited forest: {forest}"
        print(f"✅ Lamar characteristics match expectations")
    elif watershed == 'hoh':
        # Hoh is lower elevation, steep slope, dense forest
        assert elev < 1000, f"❌ Hoh should be lower elevation: {elev}"
        assert slope > 10, f"❌ Hoh should be steep: {slope}"
        assert forest > 0.7, f"❌ Hoh is temperate rainforest: {forest}"
        print(f"✅ Hoh characteristics match expectations (temperate rainforest)")
