...
def load_watershed():
    ...
    watershed_path = Path('transfer_learning/data/watershed/lamar_watershed.geojson')
...
def main():
    ...
    end_date = '2025-01-01'
    output_file = 'transfer_learning/data/climate/lamar_gridmet_elevation_bands.csv'

    # Create output directory
    Path('transfer_learning/data/climate').mkdir(parents=True, exist_ok=True)
    ...
    try:
        ...
        masks, _ = classify_elevation_bands(ds_climate, ds_elev, watershed)
        
        # Compute statistics
        df = compute_zonal_statistics(ds_climate, masks)
        ...
        for band in ['valley', 'mid', 'alpine']:
            ...
            if col in df.columns:
                plt.plot(pd.to_datetime(df['date']), df[col], label=f'{band.title()}', alpha=0.7)
        plt.title('Daily Precipitation by Elevation Band')
        plt.ylabel('mm')
        plt.legend()
        ...
        for band in ['valley', 'mid', 'alpine']:
            ...
            if col in df.columns:
                plt.plot(pd.to_datetime(df['date']), df[col], label=f'{band.title()}', alpha=0.7)
        plt.title('Daily Max Temperature by Elevation Band')
        plt.ylabel('°C')
        plt.legend()

        plt.tight_layout()
        plt.savefig('transfer_learning/data/climate/lamar_climate_elevation_bands.png', dpi=150, bbox_inches='tight')
        print(f"✓ Saved visualization to: transfer_learning/data/climate/lamar_climate_elevation_bands.png")
