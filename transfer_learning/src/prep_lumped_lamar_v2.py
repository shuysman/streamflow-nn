...
def prep_lumped_data_time():
    ...
    # Use 'date' as dimension name (generic dataset usually expects 'date' or 'time')
    # Actually, let's try to NOT have it as a coordinate if possible? No, xarray needs it.
    
    ds = xr.Dataset.from_dataframe(df)
    ds = ds.rename({'date': 'date'}) # already named date
